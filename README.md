...
```bash
sam-python312-template$ sam build --use-container
```
...
```bash
sam-python312-template$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-python312-template$ sam local start-api
sam-python312-template$ curl http://localhost:3000/
```
...
```bash
sam-python312-template$ sam logs -n HelloWorldFunction --stack-name sam-python312-template --tail
```
...
```bash
sam-python312-template$ pip install -r tests/requirements.txt --user
# unit test
sam-python312-template$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
sam-python312-template$ AWS_SAM_STACK_NAME="sam-python312-template" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "sam-python312-template"
```
